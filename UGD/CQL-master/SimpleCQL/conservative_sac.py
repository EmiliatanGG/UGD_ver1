...
from SimpleCQL.model import Scalar, soft_target_update
from SimpleCQL.utils import prefix_metrics
...
class ConservativeSAC(object):
    ...
    def cql_ratio(self,epoch):
        self.config.cql_ratio = 1 - 0.0008 * epoch

    @staticmethod
    def extract(state, action, reward, next_state, done, guide_buffer, index):
        if guide_buffer:
            for i, j in zip([state, action, reward, next_state, done], guide_buffer.keys()):
                guide_buffer[j] = np.concatenate((guide_buffer[j], i[index].detach().cpu().numpy()), axis=0)
        else:
            for i, j in zip([state, action, reward, next_state, done], ['observations','actions','rewards','next_observations','dones']):
                guide_buffer[j] =  i[index].detach().cpu().numpy()
        return guide_buffer


    def find_explore(self,state, action, reward, next_state, done, guide_buffer, new_actions, de):#仅通过欧氏距离筛选探索方向数据
        explore_index=[]
        distance = torch.sqrt(torch.sum((action - new_actions) ** 2, dim=1))
        # 计算四个 Q 函数的输出
        q1 = self.qf1(state, new_actions)
        q2 = self.qf2(state, new_actions)
        target_q1 = self.target_qf1(state, new_actions)
        target_q2 = self.target_qf2(state, new_actions)
        # 将四个 Q 函数的输出堆叠成一个张量
        q_values = torch.stack([q1, q2, target_q1, target_q2], dim=1)

        # 计算方差
        uncentain = torch.var(q_values, dim=1, unbiased=False)

        # 计算 distance * uncentain
        condition = distance * uncentain > de
        print('condition:', distance * uncentain)
        print(condition)

        # 找到满足条件的索引
        explore_index = torch.nonzero(condition).squeeze().tolist()

        # 如果 explore_index 是一个整数，转换为列表
        if isinstance(explore_index, int):
            explore_index = [explore_index]
        print('explore_index:', explore_index)
        time.sleep(1)
        return self.extract(state, new_actions, reward, next_state, done, guide_buffer, explore_index)

    def find_adv_explore(self,state, action, reward, next_state, done, guide_buffer, new_actions, q_new_actions, de):#通过欧氏距离*CQL_Q筛选探索方向数据（CQL的Q函数本身就会低估OOD数据，使用CQL_Q进一步筛选值的探索的方向）
        explore_index=[]
        for i in range(action.shape[0]):
            distance = torch.sqrt(torch.sum((action[i] - new_actions[i]) ** 2))*q_new_actions[i]
            if distance>de:
                explore_index.append(i)
        return self.extract(state, new_actions, reward, next_state, done, guide_buffer, explore_index)

    def train(self, batch, guide_buffer, epoch, bc=False):
        self._total_steps += 1
        self.cql_ratio(epoch)
        ...
        de=self.config.cql_ratio*30
        guide_buffer=self.find_explore(observations, actions, rewards, next_observations, dones, guide_buffer, new_actions, de)
        ...
        if not self.config.use_cql:
            ...
            qf_loss = qf1_loss + qf2_loss +self.config.cql_ratio*(cql_min_qf1_loss + cql_min_qf2_loss)
