...
def index_batch1(batch, indices):
    indexed = {}
    for key in batch.keys():
        indexed[key] = batch[key][indices, ...]
    return indexed

def index_batch(batch1, indices1,batch2, indices2):
    indexed = {}
    for key in batch1.keys():
        indexed[key] = np.concatenate((batch1[key][indices1,...], batch2[key][indices2,...]), axis=0)
    return indexed
...
def subsample_batch(buffer, size):
    indices = np.random.randint(buffer['observations'].shape[0], size=size)
    return index_batch1(buffer, indices)

def subsample_batch_diffusion(buffer1,buffer2, size1,size2):
    indices1 = np.random.randint(buffer1['observations'].shape[0], size=size1)
    indices2 = np.random.randint(buffer2['observations'].shape[0], size=size2)

    return index_batch(buffer1, indices1,buffer2, indices2)
