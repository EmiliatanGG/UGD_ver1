    ...
    def sample(
        ...
        for sigma, sigma_next, gamma in tqdm(sigmas_and_gammas, desc='sampling time step', mininterval=1,
            ...
            if classifier is not None:
                sa_hat = inputs_hat[:,:s_dim+a_dim].detach()
                sa_hat.requires_grad_(True)
                # 使用 sa_hat 作为分类器输入
                sa_hat=sa_hat.to(self.device)
                logits = classifier(sa_hat)
                d = torch.sigmoid(logits)
                # 计算 ∇log d - ∇log (1-d)
                grad_log = torch.autograd.grad(
                    outputs=logits.sum(),  # 直接对 logits 求导，避免 sigmoid 导数手动计算
                    inputs=sa_hat,
                    grad_outputs=None,#torch.ones(1, device=logits.device),
                    create_graph=False,  # 不需要高阶梯度
                    retain_graph=False
                )[0]
                ...
                if grad_log is not None:
                    # 创建全零的 41 维梯度模板
                    full_grad_log_d = torch.zeros_like(inputs_hat)
                    # 将 (s,a) 部分的梯度填充到对应位置
                    full_grad_log_d[:,:s_dim+a_dim] = grad_log[:,:s_dim+a_dim]
                    grad_log = full_grad_log_d

                '''grad_log_1_minus_d = torch.autograd.grad(
                    outputs=logits.sum(),
                    inputs=sa_hat,
                    grad_outputs=None,#torch.ones(1, device=logits.device),
                    create_graph=False,
                    retain_graph=False
                )[0] * (-d)  # ∇log (1-d) = ∇logits * (-d)

                if grad_log_1_minus_d is not None:
                    # 创建全零的 41 维梯度模板
                    full_grad_log_1_minus_d = torch.zeros_like(inputs_hat)
                    # 将 (s,a) 部分的梯度填充到对应位置
                    full_grad_log_1_minus_d[:,:s_dim+a_dim] = grad_log_1_minus_d[:,:s_dim+a_dim]
                    grad_log_1_minus_d = full_grad_log_1_minus_d

                # 合并修正项：∇log d - ∇log (1-d) = ∇logits * (1-d + d) = ∇logits
                correction = grad_log_d - grad_log_1_minus_d'''
                # 直接使用 ∇logits，无需额外归一化
                denoised_over_sigma -= 1.2 * grad_log###############################################################################
                sa_hat.requires_grad_(False)
            ...
            if sigma_next != 0:
                denoised_prime_over_sigma = self.score_fn(inputs_next, sigma_next, clamp=clamp, cond=cond)
                if classifier is not None:
                    sa_next_hat = inputs_next[:,:s_dim+a_dim].detach()
                    sa_next_hat.requires_grad_(True)
                    # 使用 inputs_hat 作为分类器输入
                    logits = classifier(sa_next_hat)
                    # 计算 ∇log d - ∇log (1-d)
                    grad_log = torch.autograd.grad(
                        outputs=logits.sum(),  # 直接对 logits 求导，避免 sigmoid 导数手动计算
                        inputs=sa_next_hat,
                    ...
                    correction = grad_log_d - grad_log_1_minus_d'''
                    # 直接使用 ∇logits，无需额外归一化
                    denoised_prime_over_sigma -= 1.2 * grad_log################################################################

                    sa_next_hat.requires_grad_(False)
